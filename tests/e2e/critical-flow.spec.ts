...
test("catálogos → cotización → snapshot → compartir → otro dispositivo", async ({
  ...
  await expect(page.getByTestId("quotation-total")).toHaveText("S/ 69.45");
  ...
  await expect(page.getByTestId("quotation-total")).toHaveText("S/ 299.80");
  await page
  ...
  await expect(page.getByTestId("quotation-total")).not.toHaveText("S/ 299.80");
  await page
    .getByRole("button", { name: "Editar ítem 1", exact: true })
    .click();
  ...
  await expect(page.getByTestId("quotation-total")).toHaveText("S/ 633.13");
  ...
  await expect(page.getByTestId("quotation-total")).toHaveText("S/ 522.02");
  await page.getByRole("button", { name: "Editar ítem 3", exact: true }).click();
  await page.getByLabel("Cantidad", { exact: true }).fill("3");
  await page.getByRole("button", { name: "Guardar cambios", exact: true }).click();
  await page.getByLabel("Cotizar por").selectOption("SQUARE_FOOT");
  ...
  await expect(page.locator(".ticket")).toContainText("S/ 633.13");
