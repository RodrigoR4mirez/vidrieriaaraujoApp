...
import { Eye, EyeOff, PackageOpen, Pencil, Plus, Search, Upload } from "lucide-react";
import {
  ...
  return <div className="profile-catalog-layout">
    <section className="panel glass profile-catalog-list">
      <div className="toolbar">
        ...
        <Button onClick={() => setEditing(null)}><Plus size={18} />Nuevo perfil</Button>
      </div>
      ...
      {!catalog.profiles.length ? <EmptyState title="Carga el catálogo de perfiles">
        ...
        <thead><tr><th>Imagen</th><th>Código</th><th>Descripción</th><th>Familia</th><th>Colores</th><th>Barra</th><th>Estado</th><th>Acciones</th></tr></thead>
        <tbody>{visibleProfiles.map((profile) => <tr key={profile.id} className={selected?.id === profile.id ? "selected-row" : ""}>
          <td><ProfileImage profile={profile} size={54} /></td>
          <td><button className="profile-select-button" aria-pressed={selected?.id === profile.id}
            onClick={() => setSelectedId(profile.id)}><strong>{profile.code}</strong></button></td>
          <td>{profile.description}</td><td>{familyName(profile.familyId)}</td>
          <td><div className="swatch-stack">{profile.colorPrices.map((price) => {
          ...
          <td>{profile.barLengthMeters} m</td><td><StatusBadge status={profile.status} /></td>
          <td><div className="row-actions">
            ...
            <button className="icon-button" disabled={pending}
              ...
              onClick={(event) => { event.stopPropagation(); startTransition(async () => {
              ...
              }); }}>{profile.status === "ACTIVE" ? <EyeOff size={17} /> : <Eye size={17} />}</button>
          </div></td>
        </tr>)}</tbody>
    ...
    {selected && <aside className="panel glass profile-detail">
      <h2>Detalle del perfil</h2><ProfileImage profile={selected} size={240} />
