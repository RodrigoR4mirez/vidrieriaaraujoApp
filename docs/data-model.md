...
El catálogo inicial versionado se deriva del Excel local `catalogos-fisicos/catalogo de perfiles de aluminio.xlsx`/`Hoja1`: 21 familias, 2 colores y 154 perfiles; 134 tienen imagen por código y 20 no tienen imagen de origen. Las dos filas del código físico `5220` se representan como `5220-MATE` y `5220-NEGRO`, coherentes con el JSON generado. Las imágenes se guardan como `public/profiles/<CÓDIGO>.<extensión>`; no se crean `image*.png` ni se usan renombrados Git para asignarlas. Solo la acción de carga inicial desde la web exige un catálogo de aluminio vacío; las importaciones autorizadas siguen el runbook de datos.
