...
Cada lectura de servidor exige sesión. Las lecturas de visualización (`catalog.load`, `aluminum.load`, `quotations.list/find/nextNumber`) pasan por el Data Cache de Next (`unstable_cache`, `src/application/container.ts`), server-side, nunca el navegador; las que reconcilian una escritura (dentro de `mutate` y de `confirm`, incluida la comprobación de idempotencia y la numeración de folios) siguen consultando Blob directo, sin caché de SDK, para no arriesgar condiciones de carrera ni folios duplicados. Cada Server Action de escritura (`src/app/actions.ts`) llama `updateTag` sobre el tag correspondiente (`catalog`, `aluminum-catalog`, `quotations`) al terminar, con semántica "read-your-own-writes" de Next 16: la siguiente lectura, en cualquier dispositivo, espera el dato fresco en vez de servir uno obsoleto. La navegación cliente puede reutilizar una pantalla durante 30 segundos; la acción `Actualizar` solicita datos recientes. Las páginas protegidas son dinámicas. Las Server Actions vuelven a autenticar, validan con Zod y revalidan las vistas después de guardar. El endpoint PDF responde 401 sin sesión y usa Cache-Control privado/no-store.
